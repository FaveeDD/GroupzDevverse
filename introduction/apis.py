    ...
    if request.user.is_authenticated:
        if request.method == "POST":
            ...
            if len(test_bench1) > 4:
                return JsonResponse(
                    {"message": "too many inputs in Html\n Try again"}, status=400
                )
            ...
            if outputs == correct_output1:
                ...
                return JsonResponse(
                    {"message": "Testbench failed, Code is not working\n Try again"},
                    status=200,
                )
            ...
            if outputs == correct_output2:
                return JsonResponse(
                    {
                        "message": "Congratulation, you have written a secure code.",
                        "passed": 1,
                    },
                    status=200,
                )
            ...
            return JsonResponse(
                {"message": "Test bench passed but the code is not secure"},
                status=200,
                safe=False,
            )
        else:
